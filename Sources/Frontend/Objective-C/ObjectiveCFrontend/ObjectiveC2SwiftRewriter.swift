...
public final class ObjectiveC2SwiftRewriter {
    private static var _parserStatePool = ObjcParserStatePool()
    ...
    public var astRewriterPassSource: ASTRewriterPassSource
    ...
    public init(
        ...
        output: WriterOutput,
        intentionPassesSources: IntentionPassSource? = nil,
        ...
        self.intentionPassesSource =
            intentionPassesSources ?? ArrayIntentionPassSource(intentionPasses: [])
        self.astRewriterPassSource =
        ...
        self.typeSystem = IntentionCollectionTypeSystem(intentions: intentionCollection)

        self.typeMapper = DefaultTypeMapper(typeSystem: self.typeSystem)
            ...
            queue.addOperation {
                autoreleasepool {
                    ...
                    case .globalVar(let initializer, let decl):
                        ...
                        // NOTE: - "global variable" 是什么含义？对应 ANTLR 的哪个节点？
    ...
    private func performIntentionAndSyntaxPasses() {
        ...
        let syntaxPasses =
            [MandatorySyntaxNodePass.self]
                + astRewriterPassSource.syntaxNodePasses
    ...
    private func loadObjcSource(from source: InputSource, index: Int, mutex: Mutex) throws {
        ...
        if let parserCache = parserCache {
            ...
            let processedSource = applyPreprocessors(source: src)

            parser = ObjcParser(string: processedSource, fileName: src.filePath, state: state)
